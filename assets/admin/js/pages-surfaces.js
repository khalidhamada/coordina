...
function myWorkNotificationSummary(items) {
	...
	return `<ul class="coordina-work-list coordina-work-list--compact coordina-my-work-update-list">${visible.map((item) => `<li><article class="coordina-my-work-row ${item.is_read ? '' : 'coordina-my-work-row--accent '}coordina-my-work-row--compact"><div class="coordina-my-work-row__main"><div class="coordina-my-work-row__top"><div class="coordina-my-work-row__title"><strong>${escapeHtml(item.title || __('Update', 'coordina'))}</strong></div><div class="coordina-my-work-row__badges">${!item.is_read ? `<span class="coordina-my-work-reason coordina-my-work-reason--accent">${escapeHtml(__('Unread', 'coordina'))}</span>` : ''}</div></div><p class="coordina-my-work-row__note">${escapeHtml(shortText(item.body || __('No message body yet.', 'coordina'), 120))}</p><div class="coordina-work-meta coordina-my-work-row__meta"><span>${escapeHtml(dateLabel(item.created_at))}</span></div></div></article></li>`).join('')}</ul>`;
}
