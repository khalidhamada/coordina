...
function workspaceBoard() {
	...
	const configuredStatuses = Array.isArray(shell.statuses && shell.statuses.tasks) ? shell.statuses.tasks : [];
	const extraStatuses = items.map((item) => String(item.status || '')).filter((status) => status && !configuredStatuses.includes(status));
	const statuses = configuredStatuses.concat(extraStatuses);
	const buckets = statuses.map((status) => ({ status, items: [] }));
	items.forEach((item) => {
		const bucket = buckets.find((candidate) => candidate.status === String(item.status || ''));
		if (bucket) {
...
function taskPage() {
	...
	void headerActions;
	return `<section class="coordina-page coordina-task-page"><div class="coordina-task-page__layout"><div class="coordina-task-page__main">${descriptionCard}${editSection}${updatesCard}${activityCard}</div><div class="coordina-task-page__side">${checklistPanel}${filesCard}</div></div></section>`;
}
