...
function pageHeading(pageKey, actions, fallback) {
	...
	const purpose = meta.priority === 'secondary'
		? `<span class="coordina-page-kicker">${escapeHtml(__('Support page', 'coordina'))}</span>`
		: meta.priority === 'primary'
			? `<span class="coordina-page-kicker">${escapeHtml(__('Main page', 'coordina'))}</span>`
			: '';
...
function workspaceWorkTab(taskSummary) {
	...
	return `<section class="coordina-card"><div class="coordina-section-header"><div><h3>${escapeHtml(__('Project work', 'coordina'))}</h3><p class="coordina-section-note">${escapeHtml(__('See tasks, progress, blockers, and what to do next.', 'coordina'))}</p></div>${viewButtons}</div>${summary}${body}</section>`;
}
...
function settingsPage() {
	...
	const helper = `<section class="coordina-card coordina-card--notice"><div class="coordina-section-header"><h3>${escapeHtml(__('How to use Settings', 'coordina'))}</h3></div><p>${escapeHtml(__('Start with Defaults, Intake & access, and Governance. Project-specific rules still belong inside each project workspace.', 'coordina'))}</p></section>`;
